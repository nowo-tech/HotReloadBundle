...
| ID | Story |
| --- | --- |
| US-01 | **As a** Symfony integrator, **I want** to install HotReloadBundle in `dev` **so that** HTML pages receive FrankenPHP Hot Reload client assets automatically. |
| US-02 | **As an** integrator, **I want** stable `nowo_hot_reload` keys and defaults **so that** upgrades remain predictable (see [`CONFIGURATION.md`](CONFIGURATION.md)). |
| US-03 | **As an** integrator, **I want** Twig `{{ nowo_hot_reload_assets() }}` when `auto_inject` is false **so that** I control where assets appear in the layout. |
...
**Goal:** Inject FrankenPHP (Dunglas) Hot Reload client assets into Symfony HTML responses in development.
...
- Implementing the FrankenPHP server-side Hot Reload / Mercure stack (Caddyfile is integrator-owned).
